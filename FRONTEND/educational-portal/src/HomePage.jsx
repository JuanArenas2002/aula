...
// Navbar transparente y en modo oscuro
const Navbar = () => {
  return (
    <AppBar
      position="fixed"
...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchActiveInstitutions = async () => {
      try {
        const response = await fetch('/api/instituciones/activas'); // Asegúrate de que esta URL coincide con tu backend
        const data = await response.json();
        setActiveInstitutions(data.count); // Actualizamos el estado con el número de instituciones activas
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <Container>
        <Typography variant="h4" sx={{ color: '#fff', marginBottom: '40px', textAlign: 'center', fontWeight: 'bold' }}>
          ¿Quiénes Somos?
        </Typography>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={6} md={4}>
            <InfoBox>
              ...
              <Typography variant="body1" sx={{ color: '#ccc' }}>
                En AulaTech, estamos comprometidos con transformar la educación a través de la tecnología, facilitando el
                aprendizaje y la gestión educativa para instituciones, profesores y estudiantes.
              </Typography>
            </InfoBox>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <InfoBox>
              ...
              <Typography variant="body1" sx={{ color: '#ccc' }}>
                Ser líderes en soluciones tecnológicas educativas, impulsando el futuro de la educación digital y garantizando
                la calidad y eficiencia en el aprendizaje.
              </Typography>
            </InfoBox>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <InfoBox>
              ...
              <Typography variant="body1" sx={{ color: '#ccc' }}>
                Proporcionamos herramientas avanzadas de gestión educativa, análisis de datos y plataformas colaborativas
                que mejoran la enseñanza y la organización institucional.
              </Typography>
            </InfoBox>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <InfoBox>
              ...
              <Typography variant="h4" sx={{ color: '#00FF00', marginBottom: '10px' }}>
                {activeInstitutions}
              </Typography>
        ...
        <Box sx={{ textAlign: 'center', marginTop: '60px' }}>
          <Typography variant="h5" sx={{ color: '#fff', marginBottom: '20px', fontWeight: 'bold' }}>
            ¡Descubre cómo AulaTech puede llevar tu institución al siguiente nivel!
          </Typography>
          <StyledButton href="/contact" sx={{ backgroundColor: '#4CAF50', color: '#fff' }}>
            Contáctanos
          </StyledButton>
        </Box>
      </Container>
